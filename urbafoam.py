...
from urbafoam.Config import load_config, get_or_update_config, merge_configs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quality',
                        help='Quality of the simulation, currently supports (q)uick or (n)ormal. Default quick')
    ...
    if args.wind_directions is not None:
        ...
        wind_directions = get_or_update_config(config, "urbafoam.wind", "wind_direction", [270])

    if args.building_model is None:
        primary_building_model = get_or_update_config(config, "urbafoam.models", "primaty_buildings", None)
    else:
        primary_building_model = get_or_update_config(config, "urbafoam.models", "primaty_buildings",
                                                      args.building_model, True)

    assert primary_building_model is not None and os.path.isfile(
        primary_building_model), "cannot find building model %s" % args.building_model
